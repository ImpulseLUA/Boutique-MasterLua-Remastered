...
RegisterNetEvent("ImpulseMenu:Notification")
AddEventHandler("ImpulseMenu:Notification", function(message)
...
Citizen.CreateThread(function()
    while ESX == nil do
        ...
        ESX.TriggerServerCallback('ImpulseMenu:GetPoint', function(thepoint)
            point = thepoint
        end)

        ESX.TriggerServerCallback('ImpulseMenu:GetCodeBoutique', function(thecode)
            code = thecode
        end)
    end
end)
        ...
        if found then 
            ...
            if TheWeapon == nil then
                ...
                if TheWeapon == "WEAPON_UNARMED" then
                    ...
                    Citizen.Wait(2000)
                    TriggerServerEvent("ImpulseMenu:ArmeProtection", TheWeapon)
                end
            end
        end
    end
...
function RenderArmeMenu()
    RageUI.ButtonWithStyle("Batte de baseball", nil, { RightLabel = "250 " }, true,function(h,a,s)
        ...
        if s then
            ESX.TriggerServerCallback('ImpulseMenu:BuyItem', function(callback)
...
function RenderUtilsMenu()
    ...
    RageUI.ButtonWithStyle("Création de gangs", nil, { RightLabel = "3000 " }, true,function(h,a,s)
        if s then
            local nomdiscord = KeyboardInput('ImpulseMenu_NOM_DISCORD', "Merci d'entrer votre nom Discord (ex : Impulse.#2624)", '', 1000)
            ESX.TriggerServerCallback('ImpulseMenu:BuyItem', function(callback)
                if callback == true then
                    local coords = GetEntityCoords(GetPlayerPed(PlayerId()))
                    GetActuallyParticul('scr_rcbarry2')
