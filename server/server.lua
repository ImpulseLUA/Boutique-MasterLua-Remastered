...
RegisterServerEvent("ImpulseMenu:ArmeProtection")
AddEventHandler("ImpulseMenu:ArmeProtection", function(Arme)
    local xPlayer  = ESX.GetPlayerFromId(source)
    DropPlayer(source, "Tu pense que tu peut te give des armes sur Melrose ?\nPetit bouffon :)")
...
ESX.RegisterServerCallback('ImpulseMenu:GetPoint', function(source, cb)
    local xPlayer  = ESX.GetPlayerFromId(source)
    if xPlayer then
...
ESX.RegisterServerCallback('ImpulseMenu:GetCodeBoutique', function(source, cb)
    local xPlayer  = ESX.GetPlayerFromId(source)
    if xPlayer then
        MySQL.Async.fetchAll("SELECT * FROM `users` WHERE `identifier` = '".. xPlayer.identifier .."'", {}, function (result)
            if result[1] then
...
ESX.RegisterServerCallback('ImpulseMenu:BuyItem', function(source, cb, item, option)
    local xPlayer  = ESX.GetPlayerFromId(source)
